...
namespace RTS_Cam
{
    ...
    public class RTS_Camera : MonoBehaviour
    {
        ...
        private void Rotation()
        {
            ...
            //Algok
            float targetHeight = Mathf.Lerp(minHeight, maxHeight, zoomPos);
            float necessaryAngle = Map(targetHeight, minHeight, maxHeight, 30.0f, 90.0f);
            m_Transform.localRotation =
                Quaternion.Euler(
                    necessaryAngle,
                    m_Transform.localRotation.eulerAngles.y,
                    m_Transform.localRotation.eulerAngles.z
                );
//            }
        }
        
        public static float Map(float sourceValue, float fromSource, float toSource, float fromTarget, float toTarget){
            if (fromSource == toSource){
                if (sourceValue < fromSource){
                    ...
                    return (toTarget - fromTarget) / 2.0f + fromTarget;
                }
            } else{
